...
import { getArticleBySlug, Article } from '@/lib/insights-data'; // Assuming this data is not yet localized
...
// TODO: Localize metadata if article titles/summaries are translated
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  const article = getArticleBySlug(params.slug); // This likely needs to be locale-aware if content is translated
  if (!article) {
    return {
...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  // const t = await getI18n(params.locale); // For UI strings like "Back to Insights"
    ...
    <PageContainer className="max-w-4xl mx-auto">
      <div className="mb-8">
        <Button variant="outline" asChild>
          <Link href="/our-insights"> {/* TODO: ensure this link is locale-aware if middleware uses 'prefix' strategy */}
            <ChevronLeft className="mr-2 h-4 w-4" />
            Back to Insights {/* TODO: t('insights.back_to_insights') */}
          </Link>
        </Button>
      </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs flex items-center">
                    <Edit3 className="h-3 w-3 mr-1.5" /> Published {/* TODO: t('insights.published') */}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <CalendarDays className="h-4 w-4" />
              {/* Dates might need localization if format differs */}
...
//   // This would also need to be locale-aware if content slugs differ by locale
